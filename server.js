...
async function seedCompanyServices() {
  ...
  /* تقصير الأوصاف الطويلة على البطاقات (شرح كبير ببطاقة) — يصير فقط إذا النص ما زال نص البذرة الأصلي،
     أي تعديل يدوي من الإدارة ما ينلمس */
  const shortenDesc = async (title, olds, next) => {
    for (const o of olds) {
      await db.query('UPDATE company_items SET description = ? WHERE title = ? AND description = ?', [next, title, o]);
    }
  };
  try {
    await shortenDesc('تصميم ملابس',
      ...
      'تصميم ملابس خاصة على السيرفر — كل التفاصيل والتسعير داخل نافذة الطلب.');
    await shortenDesc('بيع المركبات',
      ['شركة والتون تبيع مركبات للزبائن على السيرفر — قدّم طلبك بنوع المركبة اللي تبيها وميزانيتك، والإدارة تراجع الطلب وتتواصل معك لتجهيز المركبة والتسعير.'],
      'نوفّر ونبيع للزبون المركبة اللي يبيها على السيرفر — كل التفاصيل داخل نافذة الطلب.');
    await shortenDesc('تلت',
      ['خدمة تلت السيارات من شركة والتون — قدّم طلبك مع بيانات سيارتك ونوع التلت المطلوب، والإدارة تراجع الطلب وتتصل فيك للتنفيذ.'],
      'خدمة تلت السيارات — كل التفاصيل داخل نافذة الطلب.');
  } catch (e) { console.log('⚠️ shorten descriptions:', e.message); }
